      ...
      case "IntentRequest":
        ...
        switch(event.request.intent.name) {
          ...
          function init(){
            ...
            var endpoint = "http://mybusnow.njtransit.com/bustime/eta/getStopPredictionsETA.jsp?route="+busId+"&stop="+stopId+"&key=0.35774394008078714";
            var body = ""
            https.get(endpoint, (response) => {
              response.on('data', (chunk) => { body += chunk })
              response.on('end', () => {parseString(body, handler) })
            });
          }
          ...
          function handler(err, result) {
            ...
            context.succeed(
            generateResponse(buildSpeechletResponse(response_message, true), {} )
          )
          }
...
function createObj(b){
				return  {"time":b.pt[0],"name":b.fd[0],"minute":b.pu[0]};
			}
// Helpers
